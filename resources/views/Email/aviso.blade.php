<!DOCTYPE html>
<html>

<head>
    <title>¡Urgente! Aviso de Valores Críticos en Incubadoras</title>
</head>

<body>
    <div style="width: 100%; max-width: 600px; margin: 0 auto;">
        <div style="background-color: #ff6666; padding: 20px; border-radius: 10px;">
            <h2 style="margin-bottom: 20px; text-align: center; color: #fff;">¡Urgente! Aviso de Valores Críticos en Incubadoras</h2>
            <p style="color: #fff; font-weight: bold;">ATENCIÓN: Acción inmediata requerida</p>
            <p style="color: #fff;">Estimado/a {{ $nombre->name }} {{$nombre->last_name}},</p>
            <p style="color: #fff;">Le escribimos para informarle que se han detectado valores críticos en uno o varios sensores de las incubadoras para bebés prematuros en nuestra unidad.</p>
            <p style="color: #fff;">Es de vital importancia que tome medidas urgentes para revisar las incubadoras afectadas y corregir cualquier problema que pueda comprometer la salud y seguridad de los bebés.</p>
            <p style="color: #fff;">Los detalles específicos de los valores detectados y las incubadoras afectadas se encuentran registrados en nuestro sistema. Por favor, actúe de inmediato para evitar cualquier riesgo potencial.</p>
            <p style="color: #fff;">Rogamos su pronta atención y cooperación en este asunto crítico.</p>
            <p style="color: #fff;">¡Gracias por su rápida acción!</p>
            <p style="color: #fff;">Atentamente,<br>
                SmartNest</p>
        </div>
    </div>
</body>

</html>